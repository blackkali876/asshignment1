<?php
    include('header2.php');
?>
    <body class="bg-info">
        <form method="POST">
    <div class="container bg-dark text-white mt-4 w-50">
            <h1 class="border-bottom">CHECK EVEN AND ODD NUMBER</h1>
            <div class="row p-1">
                <div class="col-3"><h2>NUMBER:</h2></div>
                <div class="col-9"><input type="number" class="form-control" placeholder="0"  name="no1"/></div>
            </div>
            <div class="row p-1">
                <div class="col-12 text-center"><input type="submit" name="check" value="check" class="bg-danger"/></div>
            </div>
             </div>
        </form>
            <?php
                if(isset($_POST["check"]))
                {
                    $i=$_POST["no1"];
                    if($i%2==0)
                    {
                        echo "<script>alert('$i Is Even Number');</script>";
                    }
                    else
                    {
                        echo "<script>alert('$i Is Odd Number');</script>";
                    }
                    echo "<script>window.location.href='dummy5.php';</script>";
                }
            ?>
<?php
    include('putter2.php');
?>   